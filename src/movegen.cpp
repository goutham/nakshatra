...
void AddPawnPromotions(const int from_index, const int to_index,
  ...
  auto add_move = [&from_index, &to_index, &move_array] (
...
void AddPawnMoves(U64 pawn_bitboard, MoveArray* move_array) {
  static_assert (side == Side::WHITE || side == Side::BLACK,
                 "Invalid side");

  constexpr int add = side == Side::WHITE ? -pawn_move_type
                                          : pawn_move_type;
...
void GeneratePawnMoves(const Board& board,
  ...
  const U64 nw_captured = side_relative::PushNorthWest<side>(pawn_bitboard) &
                          pawn_capturable;
  if (nw_captured) {
    AddPawnMoves<variant, side, NW_CAPTURE>(nw_captured, move_acc);
  }

  const U64 ne_captured = side_relative::PushNorthEast<side>(pawn_bitboard) &
                          pawn_capturable;
  if (ne_captured) {
  ...
  const U64 one_step = side_relative::PushFront<side>(pawn_bitboard) &
                       empty_bitboard;
...
U64 GenerateAttackBitBoard(const Board& board) {
  ...
  const U64 pawn_captures = PawnCaptures<side>(
                                board.BitBoard(PieceOfSide(PAWN, side)),
                                opp_bitboard | EnpassantBitBoard<side>(board));
  const U64 pawn_one_step = side_relative::PushFront<side>(pawn_bitboard) &
                            empty_bitboard;
  ...
  return pawn_captures | pawn_one_step | pawn_two_step |
         attacks(QUEEN) | attacks(ROOK) | attacks(BISHOP) |
         attacks(KNIGHT) | attacks(KING);
}
...
void GeneratePieceMoves(const Board& board,
  ...
  while (piece_bitboard) {
    const int lsb_index = Lsb1(piece_bitboard);
    U64 attack_map = attacks::Attacks(occupancy_bitboard, lsb_index, piece) &
                     ~self_bitboard;
    if (generate_captures_only) {
