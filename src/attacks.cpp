 ...
 public:
  ...
  int EdgeDistance(int index) const {
    ...
    switch (direction_) {
      ...
      case NORTH_EAST: d = std::min(inv(row), inv(col)); break;
      case NORTH_WEST: d = std::min(inv(row), col);      break;
      case SOUTH_EAST: d = std::min(row, inv(col));      break;
      case SOUTH_WEST: d = std::min(row, col);           break;
    }
    assert(d >= 0 && d <= 7);
    return d;
  }
...
class OccupancyCombiner {
 public:
  ...
  const vector<U64>& Occupancies() const {
    return occupancies_;
  }
...
void Generate(const vector<Direction>& directions,
              const int shifts[],
              const U64 magics[],
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    GenerateAttackTable(directions, i, shifts[i], magics[i],
                        &tmp_attack_table);
...
_TmpTuple _tmp_rook_tuple = []() {
  ...
  Generate(rook_directions, rook_shifts, rook_magics,
           &rook_masks, &rook_offsets, &rook_attack_table);
...
_TmpTuple _tmp_bishop_tuple = []() {
  ...
  Generate(bishop_directions, bishop_shifts, bishop_magics,
           &bishop_masks, &bishop_offsets, &bishop_attack_table);
  return std::make_tuple(bishop_masks, bishop_offsets, bishop_attack_table);
}();

const std::array<U64, 64> rook_masks =
    std::move(std::get<0>(_tmp_rook_tuple));
const std::array<U64, 64> rook_offsets =
    std::move(std::get<1>(_tmp_rook_tuple));
const vector<U64> rook_attack_table =
    std::move(std::get<2>(_tmp_rook_tuple));
