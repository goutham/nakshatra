...
TEST(EvalAntichessTest, Evaluate) {
  ...
  EXPECT_EQ(WIN, Evaluate<Variant::ANTICHESS>(&board, -INF, INF));
...
TEST(EvalSuicideTest, Evaluate) {
  ...
  EXPECT_EQ(DRAW, Evaluate<Variant::SUICIDE>(&board, -INF, INF));
  EXPECT_EQ(DRAW, EvalResult<Variant::SUICIDE>(&board));

  board = Board(Variant::SUICIDE, "8/8/8/5p2/5P2/5P2/8/8 w - - ");
  EXPECT_EQ(-WIN, Evaluate<Variant::SUICIDE>(&board, -INF, INF));
